...
<div > 
     ...
     <a-menu  mode="inline" theme="dark" :inlineCollapsed="collapsed">
        ...
        <a-menu-item v-if="!item.hidden&&item.noDropdown&&item.children&&item.children.length>0"  :key="item.path">
            <router-link class="menu-item-a" :to="item.path+'/'+item.children[0].path" :key="item.path+'/'+item.children[0].path">
                <a-icon type="block" />
                <span>{{item.children[0].name}}</span>
            </router-link>
        </a-menu-item>
    
        <a-sub-menu v-if="!item.noDropdown&&!item.hidden" :key="item.path" >
            <template slot="title">
            ...
            <template v-for="i in item.children"  v-if="!i.hidden">
            <a-menu-item :key="i.path+'/'+i.children[0].path"> 
                <router-link  class="menu-item-a"  :to="i.path+'/'+i.children[0].path" >
                {{i.children[0].name}}
                </router-link>
            </a-menu-item>
            </template>
